...
function Table({ problemSet }) {
  return (
    <div >
      <table className="mx-auto max-w-2xl  w-full rounded-none sm:rounded-md shadow-sm divide-y divide-gray-600 overflow-hidden table-auto">
        <thead className="bg-gray-600">
          <tr className="text-gray-200 text-left font-semibold text-sm uppercase">
...
const LaddersPage = () => {
  ...
  const [ laddersId ] = useLocalStorage('laddersId');
  ...
  if (!isLoading) {
    ...
    // alert(laddersId+"hello");
    if (allProblems) { // check if `result` is defined
    for (let i = 0; i < allProblems.length; i++) {
      const problem = allProblems[i];
      let flag = false;
      // alert(submissions?.result);
      if (submissions?.result) { // check if `result` is defined
    ...
    <>
      <section id='ladder'>
        <div className="g-gray-700 p-6 mx-auto max-w-2xl mb-4 rounded-none sm:rounded-md shadow-sm">
          ...
          <div className="mt-4 h-1 bg-gray-600 container rounded-none sm:rounded-full">
            <div
              style={{ width: `${solved}%` }}
