...
export default function Select({ data, selected, setSelected }) {
  return (
    <Listbox value={selected} onChange={setSelected}>
      <div className="relative mt-1">
        <Listbox.Button className="relative w-full bg-gray-600 border border-gray-500 focus:outline-none focus-visible:ring-1 focus-visible:ring-purple-500 focus-visible:border-purple-500  rounded-md px-3 py-2 text-left shadow-sm flex justify-between items-center  sm:text-sm">
        ...
        <Transition
          ...
          leaveTo="transform opacity-0 scale-95"
        >
          <Listbox.Options className="absolute z-10 w-full py-1 mt-1 mb-1 overflow-auto text-base bg-gray-600 rounded-md shadow-lg max-h-52 ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm custom-scroll">
            {data.map((data, dataIdx) => (
              <Listbox.Option
                key={dataIdx}
                className={({ active }) =>
